...
const char * const BITCOIN_CONF_FILENAME = "castorcoin.conf";
const char * const BITCOIN_PID_FILENAME = "castorcoin.pid";
...
#else
    const char* pszModule = "castorcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Castorcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Castorcoin";
#else
    // Unix
    return pathRet / ".castorcoin";
#endif
#endif
}
